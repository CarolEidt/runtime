...
void ReturnTypeDesc::InitializeStructReturnType(Compiler* comp, CORINFO_CLASS_HANDLE retClsHnd)
{
    ...
    switch (howToReturnStruct)
    {
        ...
        case Compiler::SPK_ByValue:
        {
            ...
            {
                // GetEightByteType() will take the SSEUp classifications into account so we skip those.
                if (structDesc.eightByteClassifications[i] == SystemVClassificationTypeSSEUp)
                {
                    continue;
                }
                assert(regIndex < MAX_RET_REG_COUNT);
                m_regType[regIndex] = comp->GetEightByteType(structDesc, i);
                regIndex++;
            }
